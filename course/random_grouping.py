# 利用亂數進行分組, 至多 6 人一組, 且故意將同組組員座位錯開
import math
import random
...
# 使用 generator 建立 chunks, 每 n 個學員分為一個數列
def chunks(input, n):
...
# 從修課成員名單中讀取各員學號
content = open("2016fallcpa.txt").read()
# 利用 splitlines() 拆進數列
result = content.splitlines()
# 利用 random.shuffle() 弄亂 result 數列
random.shuffle(result)
# 每 6 人分為一組
member_num = 6
# 每 6 人分為一個數列, 然後集中在 whole_list 數列中
whole_list = list(chunks(result, member_num))
# 用來放入分組學號的字串變數, 每組一行, 組員以逗點隔開
group_data = ""
# 用兩個迴圈, 組成後續定組序, 選組長, 以及排座位用的資料字串
for i in range(len(whole_list)):
    for j in range(len(whole_list[i])):
        group_data += whole_list[i][j]+","
    group_data += "\n"

# 利用輸入的 group_data 進行後續資料處理
def optionaction(group_data):
    # 最後傳回的字串為 out_string
